...
use tinydrop_core::{CompressionEngine, EngineSettings, MozJpegEngine, OxipngEngine, WebPEngine};

fn samples_dir() -> Option<PathBuf> {
    // tests/ runs with CARGO_MANIFEST_DIR pointing at crates/tinydrop-core.
...
fn mozjpeg_decodes_input_and_writes_jpeg() {
    ...
    assert_eq!(result.format, tinydrop_core::ImageFormat::Jpeg);
