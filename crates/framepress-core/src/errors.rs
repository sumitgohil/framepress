...
/// Domain errors emitted by `framepress-core`.
///
...
pub enum CoreError {
    ...
    /// A FramePress sidecar output was submitted as a new source image.
    #[error("'{0}' is already a FramePress output; select the original image instead")]
    AlreadyOptimized(PathBuf),

    /// The input file's format is not supported by FramePress.
