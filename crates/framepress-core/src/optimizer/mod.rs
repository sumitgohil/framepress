...
impl AdaptiveOptimizer {
    ...
    /// directory, suffixed with `-framepress`.
    pub fn plan_output_path(input: &Path, format: ImageFormat) -> PathBuf {
        ...
        parent.join(format!("{stem}-framepress.{}", format.extension()))
    }
...
mod tests {
    ...
    fn optimization_never_promotes_a_larger_file() {
        ...
        let output = dir.path().join("in-framepress.png");
    ...
    fn plan_output_path_preserves_stem_and_directory() {
        ...
        assert_eq!(p, Path::new("/Users/me/Pictures/foo-framepress.png"));
    }
    ...
    fn website_preset_compresses_ads1_png() {
        ...
        let dir = tempdir().unwrap();
        let output = dir.path().join("Ads1-framepress.png");
    ...
    fn png_output_stays_png_for_every_preset() {
        ...
        let output = dir.path().join("source-framepress.png");
