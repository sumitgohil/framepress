<script lang="ts">
  ...
  let paths_key = $derived(paths.filter((path): path is string => Boolean(path)).join('\u0000'));
  let available_paths = $derived(paths.filter((path): path is string => Boolean(path)));
...
{#if source}
  <img src={source} {alt} class="h-full w-full object-cover" onerror={try_next_source} />
{:else}
  <ImageIcon {size} />
{/if}
