<script lang="ts">
  ...
  function readable_period(period: string) {
    const source = period.length === 7 ? `${period}-01T12:00:00` : `${period}T12:00:00`;
  ...
  async function reveal(win: BiggestWin) {
    ...
    try { await revealItemInDir(win.output_path); } catch { /* file may have moved after data loaded */ }
  }
...
<div class="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-8 lg:px-10">
  <header class="flex flex-wrap items-end justify-between gap-4">
    ...
    <div class="inline-flex rounded-xl border border-[var(--color-border)] bg-[var(--color-muted)] p-1" aria-label="Statistics period">
      {#each ranges as option (option.key)}
        <button type="button" class="rounded-lg px-3 py-1.5 text-sm font-medium transition-colors {range === option.key ? 'bg-[var(--color-card)] text-[var(--color-foreground)] shadow-sm' : 'text-[var(--color-muted-foreground)] hover:text-[var(--color-foreground)]'}" aria-pressed={range === option.key} onclick={() => choose_range(option.key)}>
          {option.label}
        </button>
      {/each}
    </div>
  ...
  {#if loading}
    <div class="glass flex min-h-80 items-center justify-center gap-3 rounded-2xl text-sm text-[var(--color-muted-foreground)]"><LoaderCircle size={18} class="animate-spin" /> Loading statistics…</div>
  {:else if error || !analytics}
    <div class="glass rounded-2xl p-8 text-center">
      <p class="font-medium">Statistics are unavailable</p>
      <button type="button" class="mt-3 text-sm font-medium text-[var(--color-brand-400)]" onclick={load}>Try again</button>
    </div>
  {:else}
    <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Summary statistics">
      <div class="glass rounded-2xl p-4"><p class="text-sm text-[var(--color-muted-foreground)]">Space saved</p><p class="mt-2 text-2xl font-semibold tabular-nums">{format_bytes(analytics.saved_bytes)}</p><p class="mt-1 flex items-center gap-1 text-xs {analytics.savings_change_pct !== null && analytics.savings_change_pct < 0 ? 'text-[var(--color-danger)]' : 'text-[var(--color-success)]'}">{#if analytics.savings_change_pct !== null}{analytics.savings_change_pct >= 0 ? '↑' : '↓'} {Math.abs(analytics.savings_change_pct).toFixed(0)}% vs previous period{:else}All recorded savings{/if}</p></div>
      <div class="glass rounded-2xl p-4"><p class="text-sm text-[var(--color-muted-foreground)]">Images optimized</p><p class="mt-2 text-2xl font-semibold tabular-nums">{analytics.optimized_count.toLocaleString()}</p><p class="mt-1 text-xs text-[var(--color-muted-foreground)]">Completed in this range</p></div>
      <div class="glass rounded-2xl p-4"><p class="text-sm text-[var(--color-muted-foreground)]">Average reduction</p><p class="mt-2 text-2xl font-semibold tabular-nums text-[var(--color-success)]">{analytics.average_savings_pct.toFixed(0)}%</p><p class="mt-1 text-xs text-[var(--color-muted-foreground)]">Across completed images</p></div>
      <div class="glass rounded-2xl p-4"><p class="text-sm text-[var(--color-muted-foreground)]">Original data processed</p><p class="mt-2 text-2xl font-semibold tabular-nums">{format_bytes(analytics.input_bytes)}</p><p class="mt-1 text-xs text-[var(--color-muted-foreground)]">Before optimization</p></div>
    </section>

    {#if analytics.optimized_count === 0}
      <section class="glass flex min-h-72 flex-col items-center justify-center rounded-2xl p-8 text-center">
        <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--color-brand-500)]/10 text-[var(--color-brand-400)]"><Sparkles size={26} /></div>
        <h2 class="mt-4 text-lg font-semibold">Your savings story starts here</h2>
        <p class="mt-1 max-w-sm text-sm text-[var(--color-muted-foreground)]">Optimize an image to see trends, format insights, and your biggest file-size wins.</p>
        <a href="/" class="mt-5 rounded-xl bg-[var(--color-brand-500)] px-4 py-2 text-sm font-semibold text-white">Optimize images</a>
      </section>
    {:else}
      <section class="glass rounded-2xl p-5">
        ...
        <div class="relative mt-5 h-64 overflow-hidden rounded-xl bg-[var(--color-muted)]/25">
          ...
          {#if active_point !== null && trend[active_point]}
            ...
            <div class="absolute right-3 top-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-card)] px-3 py-2 text-xs shadow-[var(--shadow-elevated)]"><p class="font-medium">{readable_period(point.period)}</p><p class="mt-1 text-[var(--color-success)]">{format_bytes(point.saved_bytes)} saved · {point.optimized_count} images</p></div>
          {/if}
        </div>
        <div class="mt-2 flex justify-between text-[11px] text-[var(--color-muted-foreground)]"><span>{trend[0] ? readable_period(trend[0].period) : ''}</span><span>{trend.at(-1) ? readable_period(trend.at(-1)?.period ?? '') : ''}</span></div>
      </section>

      <section class="grid gap-5 lg:grid-cols-2">
        <div class="glass rounded-2xl p-5"><div class="text-center"><h2 class="font-semibold">Savings by format</h2><p class="mt-1 text-sm text-[var(--color-muted-foreground)]">Which original formats shed the most weight.</p></div><div class="mt-6 flex flex-col items-center"><div class="relative grid h-32 w-32 shrink-0 place-items-center rounded-full" style:background={format_gradient}><div class="flex h-20 w-20 flex-col items-center justify-center rounded-full bg-[var(--color-card)] text-center"><span class="text-lg font-semibold leading-none">{format_bytes(format_total)}</span><span class="mt-2 text-[10px] leading-none text-[var(--color-muted-foreground)]">saved</span></div></div><div class="mt-5 grid w-full max-w-sm grid-cols-1 gap-2 sm:grid-cols-2">{#each analytics.formats as item, index (item.key)}<div class="flex items-center justify-between gap-2 rounded-xl bg-[var(--color-muted)]/70 px-3 py-2 text-sm"><span class="flex min-w-0 items-center gap-2 truncate"><i class="h-2.5 w-2.5 shrink-0 rounded-full" style:background={chart_colors[index % chart_colors.length]}></i>{item.key.toUpperCase()}</span><span class="shrink-0 text-right font-mono text-[11px] leading-4 text-[var(--color-muted-foreground)]">{format_bytes(item.saved_bytes)}<br />{format_total ? Math.round((item.saved_bytes / format_total) * 100) : 0}%</span></div>{/each}</div></div></div>
