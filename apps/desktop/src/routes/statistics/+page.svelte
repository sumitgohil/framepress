  ...
  {:else}
    ...
    {:else}
      ...
      <section class="grid gap-5 lg:grid-cols-2">
        <div class="glass rounded-2xl p-5"><h2 class="font-semibold">Savings by format</h2><p class="mt-1 text-sm text-[var(--color-muted-foreground)]">Which original formats shed the most weight.</p><div class="mt-6 flex flex-col items-center justify-center gap-5 sm:flex-row sm:gap-8"><div class="relative grid h-32 w-32 shrink-0 place-items-center rounded-full" style:background={format_gradient}><div class="grid h-20 w-20 place-items-center rounded-full bg-[var(--color-card)] text-center"><span class="text-lg font-semibold">{format_bytes(format_total)}</span><span class="text-[10px] text-[var(--color-muted-foreground)]">saved</span></div></div><div class="w-full max-w-60 space-y-3">{#each analytics.formats as item, index (item.key)}<div class="flex items-center justify-between gap-3 text-sm"><span class="flex min-w-0 items-center gap-2 truncate"><i class="h-2.5 w-2.5 shrink-0 rounded-full" style:background={chart_colors[index % chart_colors.length]}></i>{item.key.toUpperCase()}</span><span class="shrink-0 font-mono text-xs text-[var(--color-muted-foreground)]">{format_bytes(item.saved_bytes)} · {format_total ? Math.round((item.saved_bytes / format_total) * 100) : 0}%</span></div>{/each}</div></div></div>
